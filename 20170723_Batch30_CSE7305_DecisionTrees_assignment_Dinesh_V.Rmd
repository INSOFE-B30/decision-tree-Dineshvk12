...
```{r}
sum(is.na(data))
data_imputed <- knnImputation(data = data,k=5)
...
library(caret)
set.seed(123)
TrainIndex = sample(1:nrow(data_imputed), 0.7*nrow(data))
...
```{r}
set.seed(1234)
trainIndex = sample(1:nrow(building_data), 0.7*nrow(building_data))
train_reg = building_data[trainIndex, ]
test_reg = building_data[-trainIndex,]
